...
server <- function(input, output, session) {
  ...
  ### Functions
  # NOTE: inspect_data() is now provided by the BayesianStatisticalIntervalsCalculator
  # package (see R/load_n_inspect_data.R) and is available via library().
  ...
  render_intervals_plot <- function(data, input) {
    ...
    tryCatch({
      ...
      # Delegate the plotting to the package function, supplying the
      # app-computed single-chain flag and dynamic x-axis title.
      return(plot_intervals(
        list(data = data, summary = NULL),
        show_reducible = input$show_reducible,
  ...
  render_intervals_plot_one_sided <- function(data, input) {
    ...
    tryCatch({
      ...
      # Delegate the plotting to the package function, supplying the
      # app-computed single-chain flag and dynamic x-axis title.
      return(plot_one_sided_intervals(
        data,
        show_reducible = input$show_reducible,
        x_axis_title = x_axis_title,
        is_single_chain = is_single_chain
